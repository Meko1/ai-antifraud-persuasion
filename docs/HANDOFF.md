...
- **`REDIS_URL` 未配置时，复盘里的全局统计（"别人打成什么样"）不出现。**
  配好后可用 `tools/stats_seed.py` 灌一批**造出来的**样本先让那一节出现，
  但脚本自己会打印"这不是真实对局"，且往共享 Redis 上灌数据要加 `--yes`，
  建议先问过所有者。
  **排行那一行 2026-09-11 起不再跟着一起消失**：它分三级，
  服务端人群百分位（门槛每场景 8 局入档，那句话自带分母）→ 本机排名
  （`static/stats.js` 的 `localRank`，第二局起）→ 整行不出现。
  在此之前它写死着一句占位文案，而三条路（Redis 没配、连不上、样本不够）
  都不会回来改那一行，于是那是一句常驻文案——降门槛与加兜底是同一笔改动
  的两半，单降门槛救不了本机与没配 Redis 的那台。
