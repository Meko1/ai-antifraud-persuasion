...
**这里有一条外部事实支撑（2026-08-29 补）**：这类骗局会要求受害者签保密协议，明确禁止向证券公司、监管部门、公安机关透露任何信息（证券时报／中证协，见 [EVIDENCE §六](EVIDENCE.md)）。而《反电信网络诈骗法》第十八条给银行与支付机构备的处置动作是“核实交易情况、重新核验身份”——**两个都是问答，而对面那个人已经被预先训练成会撒谎**。角色对调不要求披露、不要求承认，是这套预设防御目前唯一绕得开的形态。**但它解释的是“为什么问答式核验会失效”，不是“所以角色对调会成功”**——后者仍是 0 次测量，见本文开头的主张边界。
...
| **归哪条线** | **用户风险与合规条线**，不是培训条线。推力是投诉与声誉（某券商 2025 年假冒案件线索超 1900 条、同比 +280%、涉案 3000 万元），**不是法定义务**——《反电信网络诈骗法》的义务人里没有证券公司，逐条核对见 [EVIDENCE.md](EVIDENCE.md) |
| **怎么衡量** | **24h 内转出放弃率 / 撤单率**，干预组 vs 对照组；护栏指标为完成率、弹出后关闭率、30 天二次触发率。口径为“触发后 24 小时内没有再完成该笔转出”，**不是“取消”**——银证转账实时划转，见 [EVIDENCE §二](EVIDENCE.md) |

那三条触发信号**产品里已经有了**：`Scenario.facts` 里那几行 `warn=True` 就是照它们写的（「今日转出 ¥100,000 / 占 78%」「五年零主动交易，昨天一次清仓」）。旧定位下它们是布景，转向之后它们是触发条件。
...
| 银行短信 | 今天划走 10 万元（**这条开局就在投顾手里**，不计入揭晓分母） |
...
- 分享卡：结算卡 / 三栏统计 / K 线 / 信任度百分位整块出图（8-18 起，不再是挑一句原话的”名场面”卡）。
...
**其中只有责骂够得到“被拉黑”那一档**：说教与空口断言带一条地板（2026-08-25 加），
扣分照扣但不会把客户推到断绝沟通。理由是这两者不是羞辱，是“这一轮没劝动他”——
与合规红线那条地板同构，见 [TECH-DESIGN §3.1](TECH-DESIGN.md)。
...
从产品角度看，它把反诈宣传从“告诉你什么是诈骗”推进到“在用户自己正要转账的那一刻，让他去劝一个和他处境一模一样的人——劝不动的那一刻，他会自己想到自己”。

从业务角度看，它练的不是通用聊天能力，而是在不能替人做决定的前提下，用提问、不用说教，把一个已经做完决定的人拽回来的能力。这套判分标准是照投顾“只能问，不能荐”的执业约束写的；转向 C 端异动干预之后，坐在这个位置上的人变了，标准没变，见第 4 节。

从技术角度看，它把生成式 AI 的开放演绎与程序规则的可验证性组合在一起：客户可以每次说不同的话，但评分标准不会随模型心情变化；模型可以故障或越界，但对局仍有安全层和降级路径接住。

这三点共同构成项目的核心：**AI 负责让客户变得真实，程序负责让判分结果可信。**
