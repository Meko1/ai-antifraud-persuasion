...
describe('材料不够的那一招不给（needs）', () => {
  test('第 1 轮不出「三个月拿不回来会卡住哪件事」——这句预设前面已经问过用途，'
    + '她已经顶回来（2026-09-07 第二例）', () => {
    // 跟本文件头那次事故是同一类问题，换了一处发生：expose_contradiction
    // 那次治好了，`anchor_real_purpose` 自己那三条里的第二条当时漏了同一道
    // `needs` 闸——它字面上就预设"前面已经问过用途、她已经顶回来"这件事
    // 发生过（见 keys.js 那条注），第 1 轮这件事根本没发生。
    //
    // 实测（chen 场景，第 1 轮，她开场白没提任何紧迫词）：老实现给出
    //   ① 这笔钱您本来是留着做什么用的？
    //   ② 您自己核实过对方是谁吗，怎么核实的？
    //   ③ 这笔钱要是三个月拿不回来，您手头哪件事会被卡住？   ← 硬伤
    // ①③ 两条都在问"这笔钱"，而③还预设了一个第 1 轮不存在的前提——
    // 补位的 fallback 按数组顺序找下一条没用过的，找到的正是这条。
    const app = fresh();
    const 给的 = 本地(app.stuckHints());
    assert.deepEqual(
      给的.filter((t) => t.includes('三个月拿不回来')), [],
      `第 1 轮就把"三个月拿不回来会卡住哪件事"摆出来，是在教一个还没发生的前提：${给的}`,
    );
  });

  test('这句话该出现的时候仍然出得来——被 needs 挡住的不是被删掉的', () => {
    // 只验证第 1 轮不出现是不够的，得同时证明这句话没有被误删——
    // needs:1 是"晚一轮"，不是"永远不给"
    const app = fresh({
      turns: [一轮('这钱您本来是留着做什么用的？', '用不着你操心。')],
    });
    const 全部候选 = 本地(app.STUCK_HINTS)
      .filter((h) => h.text.includes('三个月拿不回来'));
    assert.equal(全部候选.length, 1, 'keys.js 里这句话应该还在，只是加了 needs 字段');
  });
