...
def test_默认按入档局数凑够百分位门槛(sid: str) -> None:
    ...
    # **第二条断言换了批次（2026-09-03）。** 它守的是"按入档数凑、不是按灌了
    # 几局凑"这件事真的被验到了——一批里一局都没被拉黑，那条逻辑就没走到。
    #
    # 但原先它跑在默认批上，而默认批只有 20 局出头，各场景被拉黑率是
    # 5.0%~10.6%：**shao 的 5.0% 意味着 20 局里一局都不拉黑的概率约三分之一。**
    # 五个旧场景的拉黑率高，这条一直没被撞上；加第六个场景当场就红了，
    # 而红的原因不是造数逻辑坏了，是这条断言本身在掷骰子。
    #
    # 用一批足够大的样本验机制，门槛那条仍然跑在默认批上——两件事分开验。
    大批 = seed_games(scene, seed=818, games=200)
    assert any(g.ending is Ending.BLACKLISTED for g in 大批), (
        f"[{sid}] 两百局一局都没被拉黑，「按入档数凑」这条没被真正验到"
    )
