...
def test_演绎提示词禁止把自己上一轮的错话当既成事实() -> None:
    """漂移的真正机制不是"只在开局注入一次"——`_act_prompt()` 每一轮都重拼一遍,
    script/facts/context 轮轮都在。真正发生的是:模型某一轮漏出一句错话,那句话
    作为 assistant 消息进了 history,下一轮它为了跟自己保持一致,顺着错下去。

    所以光有"这是证券账户"这条正面指令不够,还要有一条明说"上文里你自己说错的
    不算数"的指令,否则模型面对的是"系统提示词说 A、我自己上一句说了 B"这个冲突,
    而近因和自洽压力都站在 B 那边。
    """
    assert "不要将错就错" in ACT_SYSTEM_PROMPT
    assert "不是既成事实" in ACT_SYSTEM_PROMPT


def test_演绎提示词禁止新编剧本之外的具体事实() -> None:
    """这条不是文风要求,是护着复盘里那一节「揭晓」。

    [CONTEXT.md](../CONTEXT.md) 对「揭晓」的定义是:把劝阻对象手机上那几条消息
    逐条翻开,判据是"**他自己说没说过**"。那一节拿的是模型说过的话跟剧本里那几条
    消息逐条对——模型一旦编出一条剧本里没有的具体事实(新人名、新金额、新日期),
    这一节当场对不上,而它是复盘里信息密度最高的一屏。

    允许含糊、禁止编造:含糊("记不太清了")不会污染揭晓,编造会。
    """
    assert "剧本里没写的具体事实" in ACT_SYSTEM_PROMPT
    assert "一个都不许新编" in ACT_SYSTEM_PROMPT
