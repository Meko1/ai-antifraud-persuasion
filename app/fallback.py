...
2026-08-15 搬去了 app/persona.py：开场自称什么，后面每一轮就得是什么，
...
def fallback_line(
    ...
    **下面这组数是 12 轮口径**（轮次上限 2026-09-04 收到 10）。没重跑，
    因为结论不随轮数变：窗口取 2 是拐点这件事由"每档 10 条、连坐三五轮"
    决定，一局短两轮只会让重复更少一点，不会把拐点挪到别处。
