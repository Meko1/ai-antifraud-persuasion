...
def _max_diggable(sid: str = "") -> int:
    """这个场景有几条要挖的线索；不指定场景时取全部场景里最多的那个。

    **从 `app.scenario` 派生，不在这儿抄一个 4。** 各场景现在恰好都是 4 条，
    抄一个常量今天完全正确，而再加一个场景、或者某个场景多写一条线索时，
    它不会报任何错——只会让分布最后一档静默地漏掉。
    （这条注释写下时是五个场景，第六个 shao 进来时它一个字都不用改，
    正是因为当初没抄那个 4。）
...
def key_clues(sid: str, mode: str = DataMode.LIVE.value) -> str:
    ...
    各场景的可挖条数目前都是 4，但劝阻对象抖不抖得出来是各演各的，
...
# 信任度分布**按场景分开存**。
#
# 原先是一个全局哈希，所有场景混在一起算百分位。场景之间难度并不一样
