...
#
# 2026-09-05 精简：原文「对方是虚构客户，机构与人物均为虚构。本内容不构成
# 投资建议；」把"虚构"说了两遍（先说客户虚构，再说机构与人物虚构），合并成
# 一句列举。三个判断点（Test告知）钉的是子串「虚构」「投资建议」「大模型」，
# 不是整句原文，这里都还在，压缩安全。
DISCLOSURE_FICTION = "对方、机构与人物均为虚构，不构成投资建议；"

# 数据去向那一句**随离线模式换**，不是在后面补一句。
# 原先是拼接，于是离线演示时页面上先说"你输入的内容会发送至大模型"、
# 再说"不调用大模型"——两句话自相矛盾，而用户只会记住错的那一句。
DISCLOSURE_LLM = "你输入的内容会发送至大模型生成回复。"
...
def build_entry(
    ...
    **内容**。转向之后触发这段对话的是一条真实的资产异动，把那条异动的详情
    ...
    同一个人玩两局是两个 gid，跨局关联不起来。留着它只为把同一局的十轮
