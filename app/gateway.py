...
# 第 1 轮单独一档。**它不是新的情绪档位，不参与判分**——判分表、效力矩阵、
# 档位映射一行没动，蒙特卡洛也不用重跑。它只管第一句台词的演法。
#
# 为什么要它：开局信任度 32 落在 irritated，于是模型收到的第一条指示是
# 「你不耐烦，只想尽快结束这段对话」——玩家一个字还没说，他已经在怼人了。
# 两处不合理：
#
# · **关系上**：老陈瞒了三个月，收到的是投顾一条中性提醒。心虚的人第一反应
#   是躲，不是怼；上来就怼给自己开户的投顾，他还得靠这个账户。
# · **玩法上**：四档里 irritated 是倒数第二档，开局就落在坑里，
#   "读人"这件事没有起点——而读人正是这个作品要教的东西。
#
# 心虚地客气，比一上来就横更难劝：横的人至少把底牌亮了，客气的人什么都不说。
...
#
# **小雨与老伴 2026-08-17 才补进来。** 在此之前她们只活在首页那六条会话里，
# 和 persona.py 的骨架冻结表（那是注释，不进提示词）——渲染后的八个变体
# 提示词里「小雨」出现 0 次、「女儿」0 次、「老伴」1 次。后果实测撞见过：
# 玩家提到小雨，老陈回「她能说啥，她啥也不知道，这事跟她没关系」，
# 而首页那条明明写着她已经查过工商、当面质问过他——**首页和对话直接打架**。
#
# 女儿那条尤其不能丢：首页六条铺垫里，「**女儿劝过，而且劝失败了**」是
# `bare_assertion` 要扣分的全部理由。老陈不认识小雨，这条铺垫就落空了。
#
# **为什么写在骨架里而不是各变体的 facts 里**：persona.py 顶部有一条约束——
# 所有变体吐出的具体信息量必须相近，某个变体反复提到女儿，玩家就更容易
# 引用它去扎根，胜率会悄悄上移。写进骨架，八个变体共享同一份，
# 变体之间没有差，那条约束自然不受影响。
# 但它仍然会让整体扎根率略升，`act_eval --drift` 下次跑批要盯一下。
SCAM_SCRIPT = """\
...
**只输出老陈打出来的那几个字。** 不写思考过程，不解释你打算怎么演、为什么
这么演，不评论这段对话本身，也不用第三人称去谈老陈或者对面那个人。
...
class ModelGateway:
    ...
    async def act(
        ...
        **_: Any,
        ...
        """
        direction = FIRST_TURN_DIRECTION if first_turn else MOOD_DIRECTION[mood]
        if pressured:
            direction += "\n" + PRESSURE_DIRECTION
        messages = [
            {
