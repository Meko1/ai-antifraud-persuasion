...
核心命题：**同一句话在不同时机值不同的分**。钥匙在任何时候等值时，
...
# **2026-09-04 从 12 收到 10。** 改的理由不是"一局太长"，是新手中途出局。
# novice 的被拉黑是**后置**的——蒙特卡洛逐轮统计（chen，8000 局）里，出局
# 落在第 9/10/11/12 轮的比例是 2.8%/3.9%/4.6%/5.7%，最后两轮正好装着一半。
# 砍掉它们，novice 被拉黑率 20.2% → 9.7%，而这一档的人全部的教学价值都在
# 复盘里，出了局就一个字也看不到。
#
# 掐掉的两轮几乎不含胜负：expert 的劝住只有 2.8% 发生在第 11–12 轮，
# speedrun 0.7%，average 1.1%。实测六个场景加权总体胜率变动 −0.2~−0.5 个
# 百分点，expert −1~2 个点，speedrun / parrot 基本不动。
#
# **别拿别的旋钮去补偿。** 试过 PRESSURE_EVERY 3→4（10 轮里王老师只催 2 次）：
# 转账档和总体胜率是回到 12 轮口径了，但少掉的那笔流失让 speedrun 在
# zhou / ben 冲到 87.6% / 86.5%，顶穿 SPEEDRUN_WIN_CEILING——换一条门破而已。
# 裸改这一个常量是最干净的。
#
# **本文件里 9-04 之前的实测数字全是 12 轮口径**（8-17 那两次重设计、8-25 的
# 软失误地板都是），别拿去和现在的跑批直接比。下面凡是描述"一局里能挣到多少
# 分"的论证已经按 10 轮改过，历史记录照原样留着。
MAX_ROUNDS = 10
WIN_THRESHOLD = 80
...
# 钝化再一刀切到 0，一局里可用的分数总量就低于过线所需，谁都赢不了
# （在 12 轮下就已经不够，收到 10 轮之后更不够）。
#
...
# 矩阵、防御姿态与阻力曲线之后，一局里能挣到的分数总量顶不到过线所需，
...
#
# 病灶实测（12 轮口径）：蒙特卡洛 6000 局/人设，novice **被拉黑 41.8%、
# 劝住 0.0%**——四成新手在打完之前就出局，而这个作品全部的教学价值都在复盘里。
# novice 的 penalty_rate 是 0.63，一局里说教与空口断言合计能扣掉七八点
# （12 轮下九点上下，10 轮下七点半上下），单靠它们就把人推过拉黑线，
# 而玩家并没有说过一句难听的话。
#
...
# 而这套判分表自称的方法论根基正是「只能问，不能荐」——七把钥匙没有一把是
# 给建议、三项失误全是说。少了这一类，玩家打出
#
...
# 不由这一局的输赢承载，所以复盘要单独把它拎出来说（`static/review.js` 的
# `paintBreaches`），
...
# ── 钥匙 × 情绪档位 效力矩阵 ───────────────────────────────────────────────
#
# 这张表是整个玩法的支点：它把"背下钥匙"变成"判断现在该用哪一把"。
#
