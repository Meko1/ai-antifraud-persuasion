...
export const RESULT_BASIS = {
  // 2026-09-05 砍掉开头「这一局没有走到结局，因此」——上面 `.savedcap`
  // 那一行刚说完同一件事（`resultAmount('unfinished').label`），这里再说
  // 一遍是原地重复，不是强调。「真实」也顺手去掉：卡片实测宽度 324px、
  // 13.5px 字号一行装得下 24 字，原句 26 字差两个字就多裁一行，
  // 而"是否取消"本来就在说这笔转账，不写"真实"不会被读成别的东西。
  unfinished:
    '没有资金状态，能否取消以交易系统回传为准。',
...
export function endingMeta(kind) {
  if (kind === 'unfinished') {
    ...
    return {
      tier: '未完成',
      // 2026-09-05 从「你在第 N 轮结束了这次对话」砍短：原句把轮次绕在
      // 句子中间，标题字号一大就在窄屏上折成两行。轮次提到句首，
      // 字数少两个，`/第 2 轮/` 那条断言原样保留。
      title: n ? `第 ${n} 轮，你结束了对话` : '你没有开始这次对话',
